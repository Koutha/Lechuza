<?php
	...
	require_once ("modelos/modelo_usuario.php"); //Llamamos al modelo

	$O_usuario = new C_usuario(); //creamos una instancia de la clase usuario.
	...
	switch ($_GET["op"]){  // según la opción "op" enviado por el AJAX se procede a comparar
		...
		case 'guardaryeditar':
			...
			if (empty($id_usuario)){ //si el ID esta vació guardamos un nuevo registro
				...
				$rspta = $O_usuario->insertar($nombre_apellido,$cedula,$telefono,$email,$cargo,$login,$clavehash,$imagen,$_POST['permiso']);
				...
				$rspta = $O_usuario->editar($id_usuario,$nombre_apellido,$cedula,$telefono,$email,$cargo,$login,$clavehash,$imagen,$_POST['permiso']);
					//Dependiendo de la inserción, la variable "repta" puede ser True o false
		...
		case 'eliminar':
			...
			$rspta = $O_usuario->eliminar($id_usuario);
		...
		case 'desactivar':
			...
			$rspta = $O_usuario->desactivar($id_usuario);
		...
		case 'activar':
			...
			$rspta=$O_usuario->activar($id_usuario);
		...
		case 'mostrar':
			...
			$rspta=$O_usuario->mostrar($id_usuario);
		...
		case 'permisos':
			...
			$id=$_GET['id'];
			$marcados = $O_usuario->listarmarcados($id);
		...
		case 'verificar':
			...
			if ( json_encode($fetch) != 'false') {
		    	...
		    	$marcados = $O_usuario->listarmarcados($fetch->id_usuario);
		    	//Declaramos el array para almacenar todos los permisos marcados
		   		$valores = array();
